    ...
    //=========================================// 
      ...
      if(phase_a_work&&phase_b_work&&phase_c_work)
      {
        ...
        if (Current_diference>0){
          if (current_a*2>((current_b+current_c)+Current_diference)){error=10;}                //255=2.55 A*100 1==0,01A
          if (current_a*2<((current_b+current_c)-Current_diference)){error=11;}
          if (current_b*2>((current_a+current_c)+Current_diference)){error=12;} 
          if (current_b*2<((current_a+current_c)-Current_diference)){error=13;} 
          if (current_c*2>((current_b+current_b)+Current_diference)){error=14;} 
          if (current_c*2<((current_b+current_b)-Current_diference)){error=15;} 
        }
...
void Screen(void *argument)
{
  ...
  for(;;)
  {
    ...
    case 0:
		...
		sprintf(R,"%02d %c %01d",current_a/10,'.',current_a%10);
		ssd1306_WriteString(R,Font_7x10,White);
		...
		sprintf(R,"%02d %c %01d",current_b/10,'.',current_b%10);
		ssd1306_WriteString(R,Font_7x10,White);
		...
		sprintf(R,"%02d %c %01d",current_c/10,'.',current_c%10);
		ssd1306_WriteString(R,Font_7x10,White);
    ...
    case 3:
		...
		sprintf(R,"%04d",Reciver_capacyty);
		ssd1306_WriteString(R,Font_7x10,White);
    ...
    case 4:
		...
		sprintf(R,"%02d %c %01d",current_diference/10,'.',current_diference%10);
		ssd1306_WriteString(R,Font_7x10,White);
		ssd1306_WriteString("A  T2:",Font_7x10,White);
    ...
    case 5:
		...
		{
			case 1:
			ssd1306_WriteString("Wrong phase connection",Font_7x10,White);
