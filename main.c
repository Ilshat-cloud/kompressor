...
current,phase_control,phase_a_work=0,phase_b_work=0,phase_c_work=0,short_circut_current,dP_time,power_nom,dP_error=0,flag=0;
    ...
    if ((!error)&&(P<P_min))
    ...
    else if((error)||(P>P_max))
      {
        if (furst_run){
           ...
           Flow_nominal=((Flow_nominal<5000)&&(Flow_nominal>0))?Flow_nominal:0;
          }
        P_time_old=0;
        start_time=0;
        HAL_GPIO_WritePin(START_GPIO_Port,START_Pin, GPIO_PIN_RESET);
